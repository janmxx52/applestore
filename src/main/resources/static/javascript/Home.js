...
function updateLoginMenu() {
  const taiKhoan = MEGA_DATA.login.cols[0];
  taiKhoan.items = [];

  if (currentUser) {
    taiKhoan.items.push(`<span class="block py-1 text-gray-300">Xin chào, <b>${currentUser}</b> 👋</span>`);
    taiKhoan.items.push('<a href="/logout" class="block py-1 hover:text-brand transition">Đăng xuất</a>');
  } else {
    taiKhoan.items.push('<a href="/login" class="block py-1 hover:text-brand transition">Đăng nhập</a>');
    taiKhoan.items.push('<a href="/register" class="block py-1 hover:text-brand transition">Đăng ký</a>');
  }
}
updateLoginMenu();
...
btnBag?.addEventListener('click', () => {
  alert('🔍 Tính năng thêm giỏ hàng sẽ được thêm sau');
});
//Hero ip16
