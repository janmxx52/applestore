...
// Nạp header/footer từ file HTML partial
...
document.addEventListener("click", e => {
  if (e.target.matches("[data-shop-link]")) {
    const category = e.target.dataset.shopLink;
    location.href = `/shop/${category}.html`;
  }
});
...
document.getElementById("addToCart").addEventListener("click", () => {
  ...
  fetch("/cart/add", {
    method: "POST",
    body: formData,
    credentials: "include"   // ✅ GIỮ COOKIE ĐĂNG NHẬP
  })
    .then(res => {
      if (res.redirected) {
